...
export const Services = ({
  ...
  const filteredBarbers = barbers.filter((barber) => {
    const categoryMatched = barber.barberCategory?.categoryName
      .toLowerCase()
      .includes(seaching);

    const favorMatched = favors.some((favor) => {
      favor.translations
        .find((t) => t.language === "en")
        ?.name.toLowerCase()
        .includes(seaching);
    });
    return categoryMatched || favorMatched;
  });
  // Filter category names based on search input
  const filteredCategories = [
    ...new Set(
     filteredBarbers.map((barber) => barber.barberCategory?.categoryName)
    ),
  ];
  ...
  return (
    <form className="mt-5">
      ...
      {filteredCategories.map((category) => {
        const barbersInCategory = barbers.filter(
          (barber) => barber.barberCategory?.categoryName === category
        );
